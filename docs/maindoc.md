...
It is time to play.  After you trained the model, the file `GA_visualize.py` will create two deliverables types:  one will do inference on images and the other will do kmeans plots <sup id="a3">[3](#f3)</sup> on images.  
...
You can also download the GAweights file from the GitHub Georgia Project website.  There are two formats available.  The weights files are in HDF5, the default in Tensorflow, and ONNX, the cross-platform format.  Download the [HDF5 weights file](https://github.com/KatherineMossDeveloper/The-Georgia-Project/releases/download/v1.5.6/GAweights.h5) or the [ONNX weights file](https://github.com/KatherineMossDeveloper/The-Georgia-Project/releases/download/v1.5.6/GAweights.onnx) to the \inference folder where you downloaded the Georgia Project code on your PC.  
...
The GA_visualize.py code will then call GAkmeans.kmeans_driver.  This code will show groupings in the OpenCrystalData dataset images, using kmeans and PCA with 4 centroids.  
...
Below is a scatter plot generated by GAkmeansd3blocks.py using the D3Blocks library, which is an open-sourced project on GitHub.  I also am trying out a new color scheme here.  For more, visit [D3Blocks](https://github.com/d3blocks/d3blocks).  
