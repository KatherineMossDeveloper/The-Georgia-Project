...
The DataProcessor class will first instantiate the GA_weaviatedatabase.py WeaviateDatabase class, which contains CRUD functions for the Weaviate vector database.  For anyone new to vector databases, it is a database that is built to contain vectors and sort them by their proximity to other vectors within the same collections.  For more...TBD.
...
Below is a scatter plot generated by GA_kmeansd3blocks.py using the D3Blocks library, which is an open-sourced project on GitHub.  For more, visit [D3Blocks](https://github.com/d3blocks/d3blocks).  

<img src="../images/GA_kmeansd3blocks.png" alt="kmeans" width="1000" height="750">  
Image by author using Python code.  
